...
Driver::Driver(const InputFile* inp, const string& pname) :
  ...
  if (cstop == 999999 && tstop == 1.e99) {
    if (mype == 0) cerr << "Must specify either cstop or tstop" << endl;
    exit(1);
  }
...
void Driver::run() {
  ...
  while (cycle < cstop && time < tstop) {
    ...
    if (mype == 0 && (cycle == 1 || cycle % dtreport == 0)) {
      ...
      cout << "End cycle " << setw(6) << cycle << ", time = " << setw(11)
           << time << ", dt = " << setw(11) << dt << ", wall = " << setw(11)
           << tdiff << endl;
  ...
  if (mype == 0) {
    ...
    cout << "cycle = " << setw(6) << cycle << ",         cstop = " << setw(6)
         << cstop << endl;
    cout << "time  = " << setw(14) << time << ", tstop = " << setw(14) << tstop
         << endl;
  ...
  mesh->write(probname, cycle, time, hydro->zr, hydro->ze, hydro->zp);
