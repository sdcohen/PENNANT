...
GenMesh::GenMesh(const InputFile* inp) {
  ...
  if (meshtype.empty()) {
    if (mype == 0) cerr << "Error:  must specify meshtype" << endl;
    exit(1);
  }
  if (meshtype != "pie" && meshtype != "rect" && meshtype != "hex") {
    if (mype == 0) cerr << "Error:  invalid meshtype " << meshtype << endl;
    exit(1);
  }
  vector<double> params = inp->getDoubleList("meshparams", vector<double>());
  if (params.empty()) {
    if (mype == 0) cerr << "Error:  must specify meshparams" << endl;
    exit(1);
  }
  if (params.size() > 4) {
    if (mype == 0) cerr << "Error:  meshparams must have <= 4 values" << endl;
    exit(1);
  }
  ...
  if (meshtype != "pie")
    ...
    lenx = (params.size() >= 3 ? params[2] : 90.0) * M_PI / 180.0;
  leny = (params.size() >= 4 ? params[3] : 1.0);

  if (gnzx <= 0 || gnzy <= 0 || lenx <= 0. || leny <= 0.) {
    if (mype == 0) cerr << "Error:  meshparams values must be positive" << endl;
    exit(1);
  }
  if (meshtype == "pie" && lenx >= 2. * M_PI) {
    if (mype == 0) cerr << "Error:  meshparams theta must be < 360" << endl;
    exit(1);
  }
...
void GenMesh::generate(std::vector<double2>& pointpos,
    std::vector<int>& zonestart, std::vector<int>& zonesize,
    std::vector<int>& zonepoints, std::vector<int>& slavemstrpes,
    std::vector<int>& slavemstrcounts, std::vector<int>& slavepoints,
    std::vector<int>& masterslvpes, std::vector<int>& masterslvcounts,
  ...
  if (meshtype == "pie")
    generatePie(pointpos, zonestart, zonesize, zonepoints, slavemstrpes,
      slavemstrcounts, slavepoints, masterslvpes, masterslvcounts,
      masterpoints);
  else if (meshtype == "rect")
    generateRect(pointpos, zonestart, zonesize, zonepoints, slavemstrpes,
      slavemstrcounts, slavepoints, masterslvpes, masterslvcounts,
      masterpoints);
  else if (meshtype == "hex")
    generateHex(pointpos, zonestart, zonesize, zonepoints, slavemstrpes,
      slavemstrcounts, slavepoints, masterslvpes, masterslvcounts,
      masterpoints);

}

void GenMesh::generateRect(std::vector<double2>& pointpos,
    std::vector<int>& zonestart, std::vector<int>& zonesize,
    std::vector<int>& zonepoints, std::vector<int>& slavemstrpes,
    std::vector<int>& slavemstrcounts, std::vector<int>& slavepoints,
    std::vector<int>& masterslvpes, std::vector<int>& masterslvcounts,
  ...
  masterpoints.reserve(
    (mypey != numpey - 1) * npx + (mypex != numpex - 1) * npy + 1);
...
void GenMesh::generatePie(std::vector<double2>& pointpos,
    std::vector<int>& zonestart, std::vector<int>& zonesize,
    std::vector<int>& zonepoints, std::vector<int>& slavemstrpes,
    std::vector<int>& slavemstrcounts, std::vector<int>& slavepoints,
    std::vector<int>& masterslvpes, std::vector<int>& masterslvcounts,
...
void GenMesh::generateHex(std::vector<double2>& pointpos,
    std::vector<int>& zonestart, std::vector<int>& zonesize,
    std::vector<int>& zonepoints, std::vector<int>& slavemstrpes,
    std::vector<int>& slavemstrcounts, std::vector<int>& slavepoints,
    std::vector<int>& masterslvpes, std::vector<int>& masterslvcounts,
  ...
  for (int j = 0; j < nzy; ++j) {
    ...
    for (int i = 0; i < nzx; ++i) {
      ...
      if (gj == 0) {
        ...
        v.erase(v.begin() + 0);
      else if (gi == gnzx - 1) v.erase(v.begin() + 3);
      zonestart.push_back(zonepoints.size());
  ...
  slavepoints.reserve((mypey != 0) * 2 * npx + (mypex != 0) * 2 * npy);
  masterpoints.reserve(
    (mypey != numpey - 1) * 2 * npx + (mypex != numpex - 1) * 2 * npy + 2);
