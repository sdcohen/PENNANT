...
Hydro::Hydro(const InputFile* inp, Mesh* m) :
      mesh(m) {
...
#pragma ivdep
      for (int z = zfirst; z < zlast; ++z) {
        if (zx[z].x > (subrgn[0] - eps) && zx[z].x < (subrgn[1] + eps)
            && zx[z].y > (subrgn[2] - eps) && zx[z].y < (subrgn[3] + eps)) {
          zr[z] = rinitsub;
          ze[z] = einitsub;
        }
      }
...
#pragma omp parallel for schedule(static)
  for (int sch = 0; sch < numsch; ++sch) {
    ...
    mesh->calcVols(pxp, zxp, sareap, svolp, zareap, zvolp, sfirst, slast);
    ...
    pgas->calcStateAtHalf(zr, zvolp, zvol0, ze, zwrate, zm, dt, zp, zss, zfirst,
      zlast);
...
#pragma omp parallel for schedule(static)
  for (int sch = 0; sch < numsch; ++sch) {
    ...
    mesh->calcVols(px, zx, sarea, svol, zarea, zvol, sfirst, slast);

    // 7. compute work
    fill(&zw[zfirst], &zw[zlast], 0.);
    calcWork(sfp, sfq, pu0, pu, pxp, dt, zw, zetot, sfirst, slast);
  }  // for sch
  mesh->checkBadSides();
...
void Hydro::advPosHalf(const double2* px0, const double2* pu0, const double dt,
    double2* pxp, const int pfirst, const int plast) {
...
void Hydro::advPosFull(const double2* px0, const double2* pu0,
    const double2* pa, const double dt, double2* px, double2* pu,
...
void Hydro::calcCrnrMass(const double* zr, const double* zarea,
    const double* smf, double* cmaswt, const int sfirst, const int slast) {
...
void Hydro::sumCrnrForce(const double2* sf, const double2* sf2,
    const double2* sf3, double2* cftot, const int sfirst, const int slast) {

#pragma ivdep
  for (int s = sfirst; s < slast; ++s) {
    int s3 = mesh->mapss3[s];

    double2 f = (sf[s] + sf2[s] + sf3[s]) - (sf[s3] + sf2[s3] + sf3[s3]);
    cftot[s] = f;
  }
}

void Hydro::calcAccel(const double2* pf, const double* pmass, double2* pa,
    const int pfirst, const int plast) {
...
void Hydro::calcRho(const double* zm, const double* zvol, double* zr,
...
void Hydro::calcWork(const double2* sf, const double2* sf2, const double2* pu0,
    const double2* pu, const double2* px, const double dt, double* zw,
    double* zetot, const int sfirst, const int slast) {
...
void Hydro::calcWorkRate(const double* zvol0, const double* zvol,
    const double* zw, const double* zp, const double dt, double* zwrate,
    const int zfirst, const int zlast) {
  double dtinv = 1. / dt;
#pragma ivdep
  for (int z = zfirst; z < zlast; ++z) {
...
void Hydro::calcEnergy(const double* zetot, const double* zm, double* ze,
    const int zfirst, const int zlast) {
...
void Hydro::sumEnergy(const double* zetot, const double* zarea,
    const double* zvol, const double* zm, const double* smf, const double2* px,
    const double2* pu, double& ei, double& ek, const int zfirst,
...
void Hydro::calcDtCourant(const double* zdl, double& dtrec, char* msgdtrec,
    const int zfirst, const int zlast) {
...
void Hydro::calcDtVolume(const double* zvol, const double* zvol0,
    const double dtlast, double& dtrec, char* msgdtrec, const int zfirst,
    const int zlast) {
...
void Hydro::calcDtHydro(const double* zdl, const double* zvol,
    const double* zvol0, const double dtlast, const int zfirst,
    const int zlast) {
  ...
  calcDtVolume(zvol, zvol0, dtlast, dtchunk, msgdtchunk, zfirst, zlast);
  if (dtchunk < dtrec) {
#pragma omp critical
    {
...
void Hydro::getDtHydro(double& dtnew, string& msgdtnew) {
...
#pragma omp parallel for schedule(static)
  for (int sch = 0; sch < mesh->numsch; ++sch) {
    ...
    double eichunk = 0.;
    double ekchunk = 0.;
    sumEnergy(zetot, mesh->zarea, mesh->zvol, zm, mesh->smf, mesh->px, pu,
      eichunk, ekchunk, zfirst, zlast, sfirst, slast);
#pragma omp critical
    {
      ei += eichunk;
      ek += ekchunk;
