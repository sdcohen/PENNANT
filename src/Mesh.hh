...
public:
  ...
  void initSides(const std::vector<int>& cellstart,
      const std::vector<int>& cellsize, const std::vector<int>& cellnodes);
  void initEdges();
  ...
  void initParallel(const std::vector<int>& slavemstrpes,
      const std::vector<int>& slavemstrcounts,
      const std::vector<int>& slavepoints, const std::vector<int>& masterslvpes,
  ...
  // write mesh
  void write(const std::string& probname, const int cycle, const double time,
  ...
  void getPlaneChunks(const int numb, const int* mapbp,
      std::vector<int>& pchbfirst, std::vector<int>& pchblast);

  // compute edge, zone centers
  void calcCtrs(const double2* px, double2* ex, double2* zx, const int sfirst,
      const int slast);

  // compute side, corner, zone volumes
  void calcVols(const double2* px, const double2* zx, double* sarea,
  ...
  void calcSideFracs(const double* sarea, const double* zarea, double* smf,
      const int sfirst, const int slast);

  // compute surface vectors for median mesh
  void calcSurfVecs(const double2* zx, const double2* ex, double2* ssurf,
      const int sfirst, const int slast);

  // compute edge lengths
  void calcEdgeLen(const double2* px, double* elen, const int sfirst,
      const int slast);

  // compute characteristic lengths
  void calcCharLen(const double* sarea, double* zdl, const int sfirst,
      const int slast);

  // sum corner variables to points (double or double2)
  template<typename T>
  void sumToPoints(const T* cvar, T* pvar);

  // helper routines for sumToPoints
  template<typename T>
  void sumOnProc(const T* cvar, T* pvar);
  template<typename T>
  void sumAcrossProcs(T* pvar);
  template<typename T>
  void parallelGather(const T* pvar, T* prxvar);
  template<typename T>
  void parallelSum(T* pvar, T* prxvar);
  template<typename T>
  void parallelScatter(T* pvar, const T* prxvar);
